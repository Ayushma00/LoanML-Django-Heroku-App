...
def ohevalue(df):
    ohe_col = joblib.load(os.path.join(BASE_DIR, "assets/ohe.pkl"))
...
def approvereject(unit):
    try:
        model = load_model(os.path.join(BASE_DIR, "assets/loan_model.h5"))
        scale = joblib.load(os.path.join(BASE_DIR, "assets/scale.pkl"))
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if int(df["LoanAmount"]) < 200000:
                ...
                messages.success(
                    request,
                    "Your Loan Request exceeds Rs. 2,00,000. Make small loan request.",
                )
    form = approvalform()
